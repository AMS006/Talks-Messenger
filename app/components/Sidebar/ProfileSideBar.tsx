...
const ProfileSideBar: React.FC<data> = ({ name, email, image }) => {
    ...
    const handleChatDelete = async () => {
        ...
        if (confirmDelete && currConversation && currConversation.id) {
    ...
    const handleConversationDelete = async () => {
        ...
        if (confirmDelete && currConversation && currConversation.id) {
            await axios.delete(`/api/conversation/${currConversation.id}`)
        }
    }
    const getAllUsers = () => {
    ...
    const handleLeaveGroup = async () => {
        const confirmLeave = confirm("Are you sure you want to leave group")

        if (confirmLeave && currConversation) {
            ...
            {currConversation && currConversation.isGroup &&
                <>
                    ...
                    <div className='flex flex-col gap-4 px-3 py-2'>
                        ...
                        {user && currConversation.users.map((data) => {
                            if (data.id !== user.id)
                                return <GroupUserBox user={data} />
                        }
                        )}
                    </div>
